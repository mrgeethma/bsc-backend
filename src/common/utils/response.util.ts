...
export interface ApiResponse<T> {
  // Generic interface to standardize API responses. generic means it can work with any data type specified when the interface is used.
  success: boolean;
  statusCode: number;
  message: string;
...
export interface ValidationErrorItem {
  // Interface to represent validation error details.
...
export class ResponseUtil {
  ...
  static success<T>(
    ...
    // Method to create a successful API response. here it takes in the data payload, an optional message (defaulting to 'Success'), and an optional status code (defaulting to 200). it returns an ApiResponse object indicating a successful operation.
