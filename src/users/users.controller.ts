...
export class UsersController {
  ...
  async getProfile(@Request() req): Promise<ApiResponseType<any>> {
    // what is @Request() req does is it injects the entire request object into the method, allowing access to properties like req.user which contains the authenticated user's information extracted from the JWT token. for example, req.user = { id: 5, email: 'test@mail.com' }
    ...
    const profileWithLocalTime = DateUtil.transformDateFields(userProfile, [
      'createdAt',
      'updatedAt',
      'lastLogin',
    ]);
    return ResponseUtil.ok(
      profileWithLocalTime,
  ...
  async getAllUsers(): Promise<ApiResponseType<any[]>> {
    ...
    const usersWithLocalTime = DateUtil.transformArrayDateFields(users, [
      'createdAt',
    ]);
  ...
  async toggleActive(@Param('id') id: string): Promise<ApiResponseType<any>> {
    ...
    const userWithLocalTime = DateUtil.transformDateFields(user, [
      'createdAt',
      'updatedAt',
      'lastLogin',
    ]);
