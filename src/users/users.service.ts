...
export class UsersService {
  ...
  async create(createUserDto: CreateUserData): Promise<User> {
    ...
    const hashedPassword = await bcrypt.hash(
      ...
      saltRounds,
    ); //bcrypt.hash function takes the plain text password and the number of salt rounds as inputs and returns a promise that resolves to the hashed password.
  ...
  async resetPassword(
    userId: string,
    resetPasswordDto: { currentPassword: string; newPassword: string },
  ): Promise<User> {
